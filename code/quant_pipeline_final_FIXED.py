...
class Config:
    ...
    # Backtest portfolio controls
    bt_use_vol_scaling: bool = True
    bt_vol_floor: float = 0.10
    bt_cash_buffer: float = 0.05
    bt_max_pos_w: float = 0.20

    # Live inference (train once on all labeled history; score latest unlabeled rows)
    live_inference: bool = False
    live_top_n: int = 5
    live_min_pred: float = 0.0
    live_use_vol_scaling: bool = True
    live_vol_floor: float = 0.10
    live_cash_buffer: float = 0.05
    live_max_pos_w: float = 0.20
    live_turnover_buffer: int = 2
    live_no_trade_weight_delta: float = 0.02

    # Optional IBKR execution (fractional shares) via ib_insync
    ibkr_enable: bool = False
    ibkr_dry_run: bool = True
    ibkr_host: str = "127.0.0.1"
    ibkr_port: int = 7497
    ibkr_client_id: int = 7
    ibkr_account: str | None = None
    ibkr_order_type: str = "MKT"  # MKT or LMT
    ibkr_limit_offset_bps: float = 5.0
    ibkr_min_trade_usd: float = 25.0
...
# Build new holdings (inverse-vol scaling if enabled)
        holdings = {}
        if bool(self.cfg.bt_use_vol_scaling) and "vol_3m" in top.columns and top["vol_3m"].notna().all():
            vol = top["vol_3m"].clip(lower=float(self.cfg.bt_vol_floor))
            raw = 1.0 / vol
            w_vec = raw / raw.sum()
        else:
            w_vec = pd.Series(1.0 / len(top), index=top.index)

        # Apply risk controls: cash buffer + max position cap
        investable = 1.0 - float(self.cfg.bt_cash_buffer)
        max_w = float(self.cfg.bt_max_pos_w)
        w_vec = w_vec.clip(upper=max_w)
        w_vec = (w_vec / w_vec.sum()) * investable

        for (idx_row, r), w_i in zip(top.iterrows(), w_vec.values):
            px = float(r["price"])
            if px > 0:
                holdings[str(r["ticker"])] = (portfolio_value * float(w_i)) / px
...
# =========================
# Live inference + optional IBKR execution
# =========================

def _feature_cols_for_model(panel: pd.DataFrame) -> List[str]:
    exclude = {"target", "price"}
    cols = [c for c in panel.columns if c not in exclude]
    cols = [c for c in cols if c not in {"window_end"}]
    return cols

def _latest_rows_per_ticker(panel: pd.DataFrame, feature_cols: List[str]) -> pd.DataFrame:
    df = panel.dropna(subset=feature_cols + ["price"]).copy()
    return df.groupby(level="ticker").tail(1)

def _inv_vol_weights(df: pd.DataFrame, vol_col: str, vol_floor: float) -> pd.Series:
    vol = df[vol_col].clip(lower=float(vol_floor))
    raw = 1.0 / vol
    return raw / raw.sum()

def _cap_and_rescale(weights: pd.Series, investable: float, max_w: float) -> pd.Series:
    w = weights.clip(upper=float(max_w))
    s = w.sum()
    if s <= 0:
        raise RuntimeError("Weights sum to zero after caps.")
    return (w / s) * float(investable)

def _ensure_dir(p: str) -> None:
    os.makedirs(p, exist_ok=True)

def _load_last_live_target(run_dir: str) -> pd.DataFrame | None:
    path = os.path.join(run_dir, "live_target_portfolio_latest.csv")
    if os.path.exists(path):
        try:
...
def build_live_target_portfolio(latest_scored: pd.DataFrame, cfg: Config, run_dir: str) -> pd.DataFrame:
    df = latest_scored.copy()
    df = df[df["pred_6m"] > float(cfg.live_min_pred)].sort_values("pred_6m", ascending=False)
    if df.empty:
        raise RuntimeError("No live candidates pass live_min_pred.")

    # Turnover/banding control
    last = _load_last_live_target(run_dir)
    keep = set()
    if last is not None and {"ticker", "target_weight"}.issubset(last.columns):
        incumbents = set(last["ticker"].astype(str).tolist())
        buffer_cut = int(cfg.live_top_n) + int(cfg.live_turnover_buffer)
        buffer_names = set(df.head(buffer_cut).index.get_level_values("ticker").astype(str).tolist())
        keep = incumbents.intersection(buffer_names)

    selected = df.head(int(cfg.live_top_n)).copy()
    if keep:
        kept = df[df.index.get_level_values("ticker").astype(str).isin(list(keep))]
        selected = pd.concat([selected, kept]).sort_values("pred_6m", ascending=False)
        selected = selected[~selected.index.duplicated(keep="first")].head(int(cfg.live_top_n))

    if bool(cfg.live_use_vol_scaling) and "vol_3m" in selected.columns and selected["vol_3m"].notna().all():
        w0 = _inv_vol_weights(selected, "vol_3m", cfg.live_vol_floor)
    else:
        w0 = pd.Series(1.0 / len(selected), index=selected.index)

    investable = 1.0 - float(cfg.live_cash_buffer)
    w = _cap_and_rescale(w0, investable=investable, max_w=cfg.live_max_pos_w)

    out = selected.copy()
    out["target_weight"] = w.values
    out = out.reset_index().rename(columns={"date": "asof_date"})
    out["asof_date"] = pd.to_datetime(out["asof_date"]).dt.date.astype(str)
    return out[["asof_date","ticker","price","pred_6m","vol_3m","target_weight"]].sort_values("pred_6m", ascending=False)

def run_live_inference(panel_all: pd.DataFrame, cfg: Config, out_prefix: str) -> pd.DataFrame:
    run_dir = "live_runs"
    _ensure_dir(run_dir)

    feature_cols = _feature_cols_for_model(panel_all)

    # Labeled history
    hist = panel_all.dropna(subset=feature_cols + ["price", "target"]).copy()
    if hist.empty:
        raise RuntimeError("No labeled history to train on (after cleaning).")

    # Latest unlabeled rows (one per ticker)
    latest = _latest_rows_per_ticker(panel_all, feature_cols).copy()

    wf = WalkForwardXGB(cfg)
    model = wf._fit_xgb(hist[feature_cols], hist["target"])

    latest["pred_6m"] = model.predict(latest[feature_cols])

    # Save signals
    signals = latest.reset_index().rename(columns={"date":"asof_date"})
    signals.to_csv(f"{out_prefix}_live_signals.csv", index=False)

    # Target portfolio
    target = build_live_target_portfolio(latest, cfg, run_dir)
    target.to_csv(f"{out_prefix}_live_target_portfolio.csv", index=False)
    target.to_csv(os.path.join(run_dir, "live_target_portfolio_latest.csv"), index=False)

    return target

def ibkr_execute_target_portfolio(target: pd.DataFrame, cfg: Config) -> None:
    """Execute target weights in IBKR using fractional share orders (via ib_insync)."""
    try:
        from ib_insync import IB, Stock, MarketOrder, LimitOrder
    except Exception as e:
        raise RuntimeError("ib_insync is required. Install with: pip install ib_insync") from e

    ib = IB()
    ib.connect(cfg.ibkr_host, int(cfg.ibkr_port), clientId=int(cfg.ibkr_client_id))
    if not ib.isConnected():
        raise RuntimeError("Could not connect to IBKR (TWS/Gateway).")

    try:
        # NetLiquidation (USD)
        nl = None
        for r in ib.accountSummary():
            if (cfg.ibkr_account is None or r.account == cfg.ibkr_account) and r.tag == "NetLiquidation" and r.currency == "USD":
                nl = float(r.value); break
        if nl is None:
            for r in ib.accountSummary():
                if (cfg.ibkr_account is None or r.account == cfg.ibkr_account) and r.tag == "NetLiquidation":
                    nl = float(r.value); break
        if nl is None:
            raise RuntimeError("Could not read NetLiquidation from IBKR.")

        # Current positions
        cur = {p.contract.symbol: float(p.position) for p in ib.positions()
               if (cfg.ibkr_account is None or p.account == cfg.ibkr_account) and p.contract.secType == "STK"}

        tp = target.copy()
        tp["target_dollars"] = tp["target_weight"].astype(float) * nl
        tp["target_shares"] = tp["target_dollars"] / tp["price"].astype(float)
        tp["current_shares"] = tp["ticker"].map(cur).fillna(0.0).astype(float)
        tp["delta_shares"] = tp["target_shares"] - tp["current_shares"]
        tp["delta_usd"] = tp["delta_shares"] * tp["price"].astype(float)

        plan = tp[tp["delta_usd"].abs() >= float(cfg.ibkr_min_trade_usd)].copy()
        plan = plan.sort_values("delta_usd", ascending=False)

        print("\n=== IBKR TRADE PLAN (fractional shares) ===")
        if plan.empty:
            print("No trades (below threshold).")
            return
        print(plan[["ticker","price","current_shares","target_shares","delta_shares","delta_usd"]].to_string(index=False))

        if bool(cfg.ibkr_dry_run):
            print("\nIBKR dry-run enabled: no orders placed.")
            return

        for _, r in plan.iterrows():
            sym = str(r["ticker"])
            delta = float(r["delta_shares"])
            ...
            if str(cfg.ibkr_order_type).upper() == "MKT":
                order = MarketOrder(action, qty)
            else:
                bps = float(cfg.ibkr_limit_offset_bps) / 10000.0
                px = float(r["price"])
        ...
        print(f"Submitted {len(plan)} orders to IBKR.")
    finally:
        ib.disconnect()
...
# Keep full panel for optional live inference
    panel_all = panel.copy()

    # Walk-forward backtest requires labeled targets
    panel_labeled = panel.dropna(subset=["target", "price"])

    # Train + OOS SHAP
    wf = WalkForwardXGB(cfg)
    preds, shap_global, metrics = wf.run(panel_labeled)
